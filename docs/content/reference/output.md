...
describes what users get. Pick a format with `-o`, or let cellphones choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cellphones <command> -o table   # aligned columns for reading
cellphones <command> -o jsonl   # one JSON object per line, for piping
cellphones <command> -o json    # a single JSON array
cellphones <command> -o csv     # spreadsheet friendly
cellphones <command> -o tsv     # tab-separated
cellphones <command> -o url     # just the URL column
cellphones <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cellphones <command> --fields id,title,url
```
...
```bash
cellphones <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cellphones <command>            # a table, because this is a terminal
cellphones <command> | wc -l    # JSONL, because this is a pipe
```
